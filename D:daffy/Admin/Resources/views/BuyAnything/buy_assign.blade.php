                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading"><b>Order Summary</b></div>
                                <div class="panel-body">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Order No:</label>{{$buy_anything->order_no}}
                                        </div>
                                        <div class="col-md-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Customer Name:</label>{{$buy_anything->user->name}}
                                        </div>
                                        <div class="col-md-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Shop Name:</label>{{$buy_anything->shop_name ?? 'NA'}}
                                        </div>
                                    </div><br>
                                    <div class="row">
                                        <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Mobile:</label>{{$buy_anything->mobile ?? 'NA'}}
                                        </div>
                                        <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Shop Location:</label>{{$buy_anything->shop_location ?? 'NA'}}
                                        </div>
                                        <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Shop Latitude:</label>{{$buy_anything->shop_latitude ?? 'NA'}}
                                        </div>
                                    </div>
                                    <br>
                                    <div class="row">
                                        <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Shop Longitude:</label>{{$buy_anything->shop_longitude ?? 'NA'}}
                                        </div>
                                        <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Order Current Status:</label> <kbd>{{$buy_anything->orderStatus->name ?? 'NA'}} </kbd>
                                        </div>
                                        <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Image:</label> <img src="{{URL('storage/app/'.$buy_anything->image)}}" width="60" height="60">
                                        </div>
                                    </div>
                                    <br>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Orderd Date:</label>{{($buy_anything->created_at)->format('d-m-Y h:i A')}}
                                        </div>
                                        <!-- <div class="col-sm-4">
                                            <label class="col-xs-12 col-lg-4 col-form-label">Current Status:</label> {{$buy_anything->status ==0  ? 'InActive' :'Active'}} 
                                        </div> -->
                                    </div>
                                    <br>
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading"><b>Product Details</b></div>
                                <div class="panel-body">
                                    ...
                                    <tbody><?php
                                    if(!is_null($buy_anything->buy_items))
                                    {
                
                                        $items =json_decode($buy_anything->buy_items);
                            ...
                            <div class="panel-heading"><b>Address</b></div>
                                ...
                                {{--    <div class="col-sm-6">
                                            <div class="card">
                                                ...
                                                <div class="card-body">
                                                    @foreach($order->orderAddresses->where('type',1) as $address)
                                                        @if($address->type === 1)
                                                    <div class="row">
                                                        <div class="col-sm-6">
                                                            <label class="col-xs-6 col-lg-6 col-form-label">Name:</label>{{$address->name ?? ''}}
                            ...
                            <div class="panel-heading"><b>Status Summary</b></div>
                                <div class="panel-body">
                                    ...
                                    <div class="row shop-tracking-status">
                                        ...
                                        <div class="col-md-12">
                                            <div class="well">
                                                <form class="d-flex" method="post" action="{{URL('admin/buyanything/changeOrderStatus')}}"  enctype='multipart/form-data'>
                                                {{csrf_field()}}
                                                <div class="form-horizontal">
                                                    <div class="form-group">
                                                    <div class="row">
                                                        <label for="inputOrderTrackingID" class="col-sm-2 control-label">Order No:{{$buy_anything->order_no}}</label>
                                                            <div class="col-sm-10">
                                                            <select class="form-control" name="status" id="order_status" required>
                                                                <option value="">--Select Status--</option>
                                                                @forelse($status as $stat)
                                                                    <option value="{{$stat->slug}}" {{($stat->id == $buy_anything->order_status) ? 'selected="selected"': ''}}>{{$stat->name}}</option>
                                                                @empty
                                                        ...
                                                        <div class="col-sm-10">
                                                            <select class="form-control" name="driver" id="driver_id" required>
                                                                ...
                                                                @forelse($drivers as $driver)
                                                                    <option value="{{$driver->id}}" {{($driver->id == $buy_anything->driver_id) ? 'selected="selected"': ''}}>{{$driver->name}}</option>
                                                                @empty
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-sm-10">
                                                            <input type="datetime-local" class="form-control form-border" placeholder="Assign Date" name="assign_date" <?php if(optional($buy_anything->deliveryData)->assign_date != null) { ?> value="{{Carbon\Carbon::parse($buy_anything->deliveryData->assign_date)->format('Y-m-d\TH:i') ?? old('assign_date' , date('Y-m-d'))}}" 
                                                        <?php } else { ?> value="{{old('assign_date' , date('Y-m-d'))}}" <?php } ?> required="required">
                                                        </div>
                                                    </div>

                                                    <input type="hidden" id="order_id" name="order_id" value="{{$buy_anything->id}}">
                                                    <br>
                                                    <div class="row">
